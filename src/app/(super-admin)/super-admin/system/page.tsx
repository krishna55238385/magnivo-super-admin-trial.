...
import { Activity, CheckCircle2, AlertCircle, XCircle, RefreshCw, Server, Database, Globe, Zap, Clock, ArrowUpRight } from 'lucide-react'
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts'

const services = [
  { name: 'Magnivo AI CRM (Next.js)', url: 'app.magnivo.ai', status: 'operational', latency: 142, uptime: 99.97, region: 'Vercel · SIN' },
  { name: 'GTM Pipeline (FastAPI)', url: 'gtm.magnivo.ai', status: 'operational', latency: 218, uptime: 99.89, region: 'Render · US-East' },
  { name: 'Supabase (PostgreSQL)', url: 'supabase.magnivo.ai', status: 'operational', latency: 24, uptime: 99.99, region: 'AWS · ap-south-1' },
  { name: 'Email Worker (Engage)', url: 'worker.magnivo.ai', status: 'degraded', latency: 1840, uptime: 98.4, region: 'Railway · EU' },
  { name: 'OpenAI API', url: 'api.openai.com', status: 'operational', latency: 380, uptime: 99.80, region: 'External' },
  { name: 'Gmail API', url: 'gmail.googleapis.com', status: 'operational', latency: 210, uptime: 99.95, region: 'External' },
  { name: 'SerpAPI', url: 'serpapi.com', status: 'operational', latency: 620, uptime: 99.50, region: 'External' },
  { name: 'Interakt (WhatsApp)', url: 'api.interakt.ai', status: 'operational', latency: 440, uptime: 99.60, region: 'External' },
]
...
const statusConfig: Record<string, { label: string; color: string; icon: any; dot: string }> = {
  ...
  outage: { label: 'Outage', color: 'text-red-400 bg-red-500/10 border-red-500/20', icon: XCircle, dot: 'bg-red-400 animate-pulse' },
}
  ...
  return (
    <div className="space-y-5 max-w-[1200px]">
      ...
      <div className={`rounded-xl p-4 border flex items-center gap-3 ${
        ...
        <div>
          <p className={`text-[13px] font-semibold ${allOperational ? 'text-emerald-400' : 'text-amber-400'}`}>
            {allOperational ? 'All Systems Operational' : `${degraded.length} Service(s) Degraded`}
          </p>
          <p className="text-[11px] text-white/40 mt-0.5">
            {allOperational ? 'No incidents or degradations detected.' : degraded.map(s => s.name).join(', ') + ' is experiencing elevated latency.'}
          </p>
        </div>
            ...
            return (
              <div key={s.name} className="flex items-center gap-4 px-5 py-4 hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/30">{s.url} · {s.region}</p>
                </div>
                <div className="flex items-center gap-8">
                  <div className="text-right">
                    <p className="text-[12px] font-semibold text-white">{s.latency} ms</p>
                    <p className="text-[10px] text-white/30">Latency</p>
                  </div>
                  <div className="text-right">
                    <p className={`text-[12px] font-semibold ${s.uptime >= 99.9 ? 'text-emerald-400' : s.uptime >= 99 ? 'text-amber-400' : 'text-red-400'}`}>{s.uptime}%</p>
                    <p className="text-[10px] text-white/30">Uptime (30d)</p>
                  </div>
      ...
      <div className="bg-[#111118] border border-white/[0.07] rounded-xl p-5">
        ...
        <p className="text-[11px] text-white/30 mb-5">Response time in ms for core services</p>
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Error rates */}
        <div className="bg-[#111118] border border-white/[0.07] rounded-xl p-5">
          <p className="text-[13px] font-semibold text-white mb-4">API Error Rates (30d)</p>
        ...
        {/* Incidents */}
        <div className="bg-[#111118] border border-white/[0.07] rounded-xl p-5">
          <p className="text-[13px] font-semibold text-white mb-4">Recent Incidents</p>
          <div className="space-y-3">
            {incidents.map(inc => (
